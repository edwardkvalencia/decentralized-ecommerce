...
class Order extends Component {
    constructor() {
        ...
        // separating completed from pending based on order state
        this.state = {
            pendingSellerOrders: [],
            pendingBuyerOrders: [],
            completedOrders: [],
    ...
    async displayOrders() {
        ...
        if(this.state.pendingSellerOrders.length == 0) {
            pendingSellerOrdersHtml.push((
                <div key="0" className="center">There are no seller orders...</div>
            ))
        }

        if(this.state.pendingBuyerOrders.length == 0) {
            pendingBuyerOrdersHtml.push((
                <div key="0" className="center">There are no buyer orders...</div>
            ))
        }
